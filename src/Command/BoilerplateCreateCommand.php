...
namespace FriendsOfWp\BoilerplateDevCliExtension\Command;
...
use FriendsOfWp\BoilerplateDevCliExtension\Boilerplate\Step\Step;
...
class BoilerplateCreateCommand extends Command
{
    ...
    private function initSteps(string $configFile, Configuration $configuration, InputInterface $input, OutputInterface $output)
    {
        ...
        $config = ConfigFileLoader::loadYamlConfig($configFile, __DIR__ . '/../../config/default.yml');
    ...
    private function runSteps(OutputInterface $output): void
    {
        $stepCount = 0;
